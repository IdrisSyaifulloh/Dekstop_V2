...
class ModelUpdater:
    ...
    def __init__(self, backend_url: str = "http://localhost:8000", models_dir: str = "models"):
        """Initialize the updater with backend URL and local models directory."""
    ...
    def get_current_version(self) -> Optional[Dict]:
        """Return version info dict for the currently installed model, or None if absent."""
    ...
    def check_for_updates(self) -> Optional[Dict]:
        """Query the backend for the latest model version and compare against the installed one."""
        try:
    ...
    def download_model(
        ...
        """Stream-download the specified model version and return the temp file path."""
        try:
    ...
    def verify_model(self, file_path: Path, expected_hash: str) -> bool:
        """Return True if the file's SHA-256 hash matches the expected hash."""
        try:
    ...
    def backup_current_model(self) -> bool:
        """Copy the current model to the backup directory with a timestamp suffix."""
        try:
            current = self.get_current_version()
            if not current:
    ...
    def install_model(self, temp_file: Path, version: str, metadata: Dict) -> bool:
        """Move the downloaded temp file to its final location and persist version metadata."""
        try:
    ...
    def rollback(self) -> bool:
        """Restore the most recent backup model, returning True on success."""
        try:
            # Find latest backup
    ...
    def _cleanup_old_backups(self, keep: int = 2):
        """Delete old backup files, retaining only the most recent `keep` files."""
        try:
            backups = sorted(self.backup_dir.glob("*.bak"), reverse=True)
    ...
    def update_model(
        ...
        """Run the full update pipeline: check, download, verify, and install the model."""
        # Check for updates
...
if __name__ == "__main__":
    ...
    if update_info and update_info.get('update_available'):
        ...
        def progress(downloaded, total):
            """Print download progress percentage to stdout."""
