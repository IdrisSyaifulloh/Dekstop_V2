...
class EmbeddedBackend:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 8000):
        """Initialize the embedded backend with host, port, and database path."""
        self.host = host
        self.port = port
    ...
    def _setup_database(self):
        """Create the scan_results SQLite table if it does not already exist."""
        import sqlite3
    ...
    def _create_app(self):
        ...
        class ScanResult(BaseModel):
            """Pydantic model representing a scan result payload."""
            filename: str
            label: str
            file_hash: str = None
        ...
        def save_scan_result(scan: ScanResult):
            """Persist a scan result to the database, rejecting duplicate file hashes."""
            try:
                conn = get_db()
                cursor = conn.cursor()
        ...
        def get_scan_history(limit: int = 10):
            """Return the most recent scan results up to the requested limit."""
            try:
                conn = get_db()
                cursor = conn.cursor()
        ...
        @app.get("/stats")
        def get_stats():
            """Return total scan count grouped by classification label."""
            try:
                conn = get_db()
                cursor = conn.cursor()
    ...
    def _run_server(self):
        """Set up the database, build the FastAPI app, and run the uvicorn server."""
        import uvicorn
    ...
    def start(self):
        """Launch the embedded backend in a daemon thread and verify it responds."""
        if self.running:
    ...
    def stop(self):
        """Signal the embedded backend to stop."""
        self.running = False
        logger.info("Embedded backend stopped")

    def is_running(self) -> bool:
        """Return True if the server thread is alive and the running flag is set."""
